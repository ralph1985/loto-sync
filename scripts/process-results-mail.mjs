...
function buildHtmlReport(result, group, dateLabel, balance, summary) {
  ...
  const resultDetails = result.game === 'EUROMILLONES'
    ? `<strong>Estrellas:</strong> ${escapeHtml(result.stars.join(', ') || 'no indicadas')} · <strong>El Millón:</strong> ${escapeHtml(result.elMillionCode ?? 'no indicado')}`
    : `<strong>Complementario:</strong> ${escapeHtml(result.complementario ?? 'no indicado')} · <strong>Reintegro:</strong> ${escapeHtml(result.reintegro ?? 'no indicado')}`;
